...
public class FruitController {
    ...
    @GetMapping("/{id}")
    public FruitResponse get(@Positive(message = "Id cannot be less than zero") @PathVariable("id")Long id){
         return new FruitResponse("Get by id succeed",fruitService.getById(id));
    }

    @GetMapping("/name/{name}")
    public List<Fruit> getByName(@Size(min=2, max=45, message="Name length must be between 2 to 45")@PathVariable("name") String name){
